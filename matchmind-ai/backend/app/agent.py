"""Build the football agent and its safe per-request tools."""
from dataclasses import dataclass, field
from google.adk.agents import Agent
from google.genai import types
from app.prediction import PredictionService, UnsupportedFixture

# Version tag so prompt changes can be tracked in logs and metadata.
PROMPT_VERSION = "football-analyst-v2"
# Main instruction block that tells Gemini what it may and may not do.
INSTRUCTION = """
...
@dataclass
class ToolState:
    """Track tool calls and returned predictions for one request."""
...
def build_agent(predictor: PredictionService, model, state: ToolState, max_calls: int = 6) -> Agent:
    """Create the ADK agent plus the only tools it is allowed to use."""

    def allowed(name: str) -> bool:
        # Count calls so the request cannot exceed the tool budget.
        state.attempts += 1
