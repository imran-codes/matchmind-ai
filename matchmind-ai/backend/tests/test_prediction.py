"""Tests for the prediction service and its output validation."""
import pytest
...
def test_distribution_and_origin(predictor):
    """Verify predictions look like a proper probability distribution."""
...
def test_invalid_fixture_rejected_inside_tool_boundary(predictor, home, away):
    """Make sure invalid fixture requests fail before scoring happens."""
    with pytest.raises(ValueError):
        predictor.predict(home, away)


def test_nan_is_not_a_valid_probability(predictor):
    """Check that the response schema rejects invalid probability values."""
    result = predictor.predict("Arsenal", "Liverpool")
