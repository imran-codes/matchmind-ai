"""Tests for chronological feature construction and leakage protection."""
import pandas as pd
...
def test_features_are_created_only_after_prior_history() -> None:
    """Only matches with enough prior history should become training rows."""
    rows = []
...
def test_current_result_cannot_change_its_own_features():
    """Changing a match result should not change its own earlier feature row."""
...
def test_matches_on_same_date_cannot_see_each_others_results():
    """Fixtures on the same date must not leak results into each other."""
    from app.features import FEATURES
    rows = [{"Date": f"{i+1:02d}/01/2025", "HomeTeam": "A", "AwayTeam": "B", "FTHG":1,
