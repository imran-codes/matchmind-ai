"""Pydantic models that define API request and response shapes."""
import math
...
class PredictionRequest(BaseModel):
    """Request body for a direct home-vs-away prediction."""
    ...
    def different_teams(self):
        """Reject requests that use the same team on both sides."""
...
class Prediction(BaseModel):
    """Response body returned by the classifier and agent tool."""
    home_team: str
    away_team: str
    ...
    def valid_distribution(self):
        """Ensure the three probabilities are valid and sum to one."""
...
class AnalyseRequest(BaseModel):
    """Request body for the natural-language agent endpoint."""
    model_config = ConfigDict(extra="forbid", str_strip_whitespace=True)
    message: str = Field(min_length=5, max_length=800)


class Analysis(BaseModel):
    """Response body returned by the ADK agent route."""
    request_id: str
